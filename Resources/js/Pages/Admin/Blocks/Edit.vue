...
const form = useForm({
    store_id: props.block.store_id,
    code: props.block.code,
    is_active: props.block.is_active,
    sort_order: props.block.sort_order,
    translations: props.block.translations,
});

const submit = () => {
    form.put(route('admin.general.html.update', props.block.id));
};

const deleteBlock = () => {
    if (confirm('Are you sure you want to delete this HTML block?')) {
        form.delete(route('admin.general.html.destroy', props.block.id));
    }
};
...
<template>
    <AdminLayout :title="'Edit: ' + (block.title || 'HTML Block')">
        <template #header>
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-gray-900">Edit HTML Block</h1>
                        <p class="text-sm text-gray-500">
                            Position: {{ getPositionLabel(block.code) }}
                            <span v-if="block.store" class="ml-2">| Store: {{ block.store?.code }}</span>
                        </p>
                    </div>
                </div>
                <button
                    @click="deleteBlock"
                    class="inline-flex items-center px-4 py-2 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition"
                >
